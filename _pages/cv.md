---
layout: archive
title: ""
permalink: /cv/
author_profile: true
redirect_from:
  - /resume
---

{% include base_path %}

Education
======
* Ph.D in Business Management (Focus on Marketing and Information System), University of Manchester (UK), 2026 (expected)
* M.S. in Computer Science with Artificial Intelligence (one year taught + one year research), University of Nottingham (UK), 2022
* B.A. in Business Management, Staffordshire University (UK), 2019
* B.A. in Business Management, Chengdu University of Technology (China, Double First-Class Construction), 2019


Research experience
======
* Spring 2025: Research Assistant
  * Alliance Manchester Business School, University of Manchester
  * Analysis the research output of Alliance Manchester Business School under the research Committee of AMBS

* Fall 2022: Team Member
  * Alan Turing Institute
  * The project is lead by Alan Turing Institute. I have participated in this group to analyse the confidential data and successfully
developed the Masked Autheconder Model to do data augmentation for time series data. [Report](https://www.turing.ac.uk/news/publications/data-study-group-final-report-advanced-manufacturing-research-centre-amrc)

* Spring 2022: Research Assistant
  * The Hong Kong Polytechnic University
  * I am working with Dr Qi Zhu, and I am mainly responsible for Data processing and textual data analysis by NLP. The project mainly investigates the Conference Call data

* Fall 2021: Team Member
  * University of Nottingham
  * Face Scanner: The PICU (Paediatric Intensive Care Unit) at QMC (Queen’s Medical Centre, University of Nottingham) has requested a face scanner be developed for patients requiring NIV (Non-invasive Ventilation), where a tightly fitting mask is put on the children’s face, and this is a three-year project. This project main related with Computer Version and 3D modelling.

* Summer 2021: Research Assistant
  * University of Nottingham, China
  * Worked with Dr Boying Li from the University of Nottingham Ningbo China to investigate the social media data

  
Research Impact
======
I have developed the following public text mining tool for marketing research and practices:
* Communication Style: [Link](https://huggingface.co/CommunicationStyle/Communication_Style) (Total downloads times: 51,402, verifying download times by this [Offical API Link](https://huggingface.co/api/models/CommunicationStyle/Communication_Style?expand[]=downloadsAllTime)
* Influence Tactic: [Link](https://huggingface.co/InfluenceTactics/Influence_Tactic_Paper) (Total downloads times: 313, verifying download times by this [Offical API Link](https://huggingface.co/api/models/InfluenceTactics/Influence_Tactic_Paper?expand[]=downloadsAllTime)
* Process and Outcome: [Link](https://huggingface.co/ProcessOutcomePaper/ProcessOutcome) (Total downloads times: 87, verifying download times by this [Offical API Link](https://huggingface.co/api/models/ProcessOutcomePaper/ProcessOutcome?expand[]=downloadsAllTime)

Funding
======
* Misinformation and Large Language Model, Google Cloud research credits. Amount: $1000. From Jan 2025 to Jan 2026.
* Alliance Manchester Business School Full PhD Scholarships (4 years). From Sep 2022 to Sep 2026.
  
Technical Skills
======
* Languages: Python, R, SQL, JavaScript, HTML/CSS
* Machine Learning Framework: Pytorch, TensorFlow
  
Languages
======
* English (Advanced), Mandarin (Native), Sichuan dialect (my mother tongue, haha)
